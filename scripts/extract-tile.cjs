// Usage: node scripts/extract-tile.cjs <frame> [output.png]
// Extracts a single 64x64 tile from tileset.png by 0-based frame number.
// Output defaults to tile_<frame>.png in the current working directory.
...
const [,, frameArg, outArg] = process.argv;
if (frameArg === undefined) {
  console.error('Usage: node scripts/extract-tile.cjs <frame> [output.png]');
  process.exit(1);
}
...
if (ox + TILE_SIZE > width || oy + TILE_SIZE > height) {
  console.error(`Frame ${frame} is out of bounds for a ${width}×${height} tileset.`);
  process.exit(1);
}
...
const outFile = outArg || `tile_${frame}.png`;
fs.writeFileSync(outFile, encodePNG(TILE_SIZE, TILE_SIZE, tilePixels));
console.log(`Extracted tile #${frame} (col ${col}, row ${row}) → ${outFile}`);
