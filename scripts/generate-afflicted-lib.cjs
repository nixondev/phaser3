/**
 * Shared Afflicted Generation Logic for WARDEN.
 * Creates 4x4 afflicted spritesheets with multiple variants.
 */
...
function generateAfflicted(localS = 1, variant = 'walker') {
  const preset =
    typeof variant === 'string'
      ? (AFFLICTED_VARIANTS[variant] || AFFLICTED_VARIANTS.walker)
      : { ...AFFLICTED_VARIANTS.walker, ...variant };
  ...
  function drawCharacter(col, row, facing, walkPhase) {
    ...
    // exposed skin around neckline / tears
...
function generateVialCure() {
  ...
  // liquid fill
  fillRect(11, 15, 20, 26, 88, 196, 110);
  // glass body (semi-transparent overlay)
  fillRect(11, 10, 20, 27, 200, 235, 215, 55);
  // neck
  fillRect(13, 7, 18, 10, 200, 235, 215, 90);
  // stopper
  fillRect(13, 5, 18, 7, 210, 175, 60);
  fillRect(14, 4, 17, 5, 190, 155, 50);
  // body outline
  for (let y = 10; y <= 27; y++) { sp(10, y, 45, 110, 65); sp(21, y, 45, 110, 65); }
  for (let x = 11; x <= 20; x++) { sp(x, 10, 45, 110, 65); sp(x, 27, 45, 110, 65); }
  // neck outline
  for (let y = 7; y <= 10; y++) { sp(12, y, 45, 110, 65); sp(19, y, 45, 110, 65); }
  // highlight streak
  fillRect(12, 11, 13, 22, 255, 255, 255, 70);
  sp(12, 10, 255, 255, 255, 50);
  // bubble
  sp(17, 20, 255, 255, 255, 120);
  sp(17, 19, 255, 255, 255, 60);
